...
class SuNingSkuParser(spider.Parser):
    ...
    def htm_parse(self, priority: int, url: str, keys: dict, deep: int, content: object):
        ...
        if 'product' in url:
            ...
            item['productActualID'] = re.compile(r"(?<=ninePartNumber\":\").+?(?=\")").findall(content)[0]
            ...
            if int(url.split('/')[-2]) != 0:
                ...
                url_list.append((shop_url, {'productActualID':item['productActualID']}, priority-1))
            if item['productCompleteID'] and item['storeActualID'] :
                ...
                url_list.append((pas_url, {'productActualID':item['productActualID']}, priority-1))
                url_list.append((review_url, {'productActualID':item['productActualID']}, priority-1))
            else:
                return -1, [], []
...
class SuNingSkuSaver(spider.Saver):
    ...
    def __init__(self, config):
        ...
        self.db = self.eb['SuNing_' + str(datetime.datetime.now().month)]
        return
